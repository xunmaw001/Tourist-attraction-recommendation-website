      ...
      <el-row>
        ...
        <el-form-item class="input" v-if="type!='info'"  label="景点编号" prop="jingdianbianhao">
          <el-input v-model="ruleForm.jingdianbianhao" 
              placeholder="景点编号" clearable  :readonly="ro.jingdianbianhao"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="景点编号" prop="jingdianbianhao">
              <el-input v-model="ruleForm.jingdianbianhao" 
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `jingdiangoupiao/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jingdiangoupiaoCrossAddOrUpdateFlag = false;
                  this.parent.search();
